...
def main():
    ...
    # sample 10000 numbers between 0 ~ 5 as input
    model  = Model()
    optimizer = optim.Adam(model.parameters(), lr =0.001)
    plot_results(model)
    for step in range(10000):
        batch_x = np.random.choice(data_x, 32)
        # configure mini-batch as random 32 data
        ...
        loss = F.mse_loss(pred, truth) #calculate loss using MSE

        optimizer.zero_grad()
        loss.mean().backward() #calculate gradient using backpropagation
